...
const AdminCanvasContent: React.FC = () => {
  ...
  useEffect(() => { loadGlobalAssets(); }, []);
  useEffect(() => { if (activeHistoriaId) loadStoryGraph(activeHistoriaId); }, [activeHistoriaId]);

  // 2. Construir Grafo
  const loadStoryGraph = async (historiaId: number) => {
    setNodes([]); setEdges([]);
    ...
    if (pasos) {
        pasos.forEach((paso: any, i: number) => {
            ...
            // Recuperamos las posiciones guardadas específicamente para estos activos dentro del metadata del paso
            ...
            if (paso.id_personaje) {
                ...
                if(char) {
                     ...
                     // Si existe posición guardada, usarla. Si no, calcular relativa al paso.
  ...
  const onNodeDragStop: NodeDragHandler = useCallback(async (_: React.MouseEvent, node: Node) => {
      ...
      // Si es HISTORIA o UBICACIÓN (Tablas propias)
      if (node.type === 'story') {
      ...
      // Si es un ACTIVO SATÉLITE (Personaje, Recompensa, Multimedia conectado a un paso)
      else if (['character', 'reward', 'multimedia'].includes(node.type || '')) {
           // El ID del nodo activo tiene el formato: "tipo-ID-step-STEPID"
           // Ejemplo: char-5-step-102
           const parts = node.id.split('-step-');
           
           // Solo guardamos si está conectado a un paso (tiene la parte -step-)
           if (parts.length === 2) {
               const stepId = parseInt(parts[1]); // ID del paso padre
               
               // Obtenemos el metadata actual del paso desde Supabase o del estado local
               ...
               if (pasoData) {
                   ...
                   // Actualizamos la posición específica de este tipo de activo
                   const newPositions = {
                       ...currentPositions,
                   ...
                   // Guardamos en la tabla de FLUJO (no en la tabla de personaje)
  ...
  const handleSaveNodeData = async (nodeId: string, newData: any) => {
    ...
    // 1. Identificar Tabla e ID según el tipo de nodo
    ...
    if (table) {
        // 2. Preparar los datos para enviar a Supabase
        
        // Extraemos propiedades que NO van directas o necesitan tratamiento especial.
        // Nota: Sacamos 'tipo' aquí porque ReactFlow a veces lo usa internamente, 
        // pero abajo lo volvemos a meter si la tabla lo requiere.
        const { label, originalData, tipo, metadata, [idField]: _, ...dbData } = newData;

        let finalDbData = { ...dbData };

        // --- A. CORRECCIÓN DE TIPO (Ubicación, Recompensa, Multimedia) ---
        // Estas tablas tienen una columna explícita llamada 'tipo'.
        if (['location', 'reward', 'multimedia'].includes(node.type || '')) {
            finalDbData.tipo = tipo;
        }

        // --- B. MANEJO DE CAMPOS JSON ESPECÍFICOS ---
        
        // Caso 1: Pasos e Historia usan 'metadata' (para posiciones y opciones)
        if (['step', 'story', 'location'].includes(node.type || '')) { 
            // Mantenemos la metadata original (posiciones del canvas) y mezclamos lo nuevo
            const prevMeta = originalData.metadata || {};
            const newMeta = metadata || {};
            finalDbData.metadata = { ...prevMeta, ...newMeta };
        }

        // Caso 2: Personajes usan 'atributos_json'
        if (node.type === 'character' && newData.atributos_json) {
            finalDbData.atributos_json = newData.atributos_json;
        }

        // Caso 3: Multimedia usa 'metadatos'
        if (node.type === 'multimedia' && newData.metadatos) {
            finalDbData.metadatos = newData.metadatos;
        }

        // --- C. LÓGICA DE PASOS (Decisiones) ---
        ...
        // 4. Actualizar el Nodo Visualmente (Estado Local)
            ...
            if (n.id === nodeId) {
                return {
                    ...
                    data: {
                        ...
                        // Actualizamos el tipo para cambiar el color/icono si aplica
                        tipo: finalDbData.tipo || finalDbData.tipo_paso || n.data.tipo, 
                        // 🔥 CORRECCIÓN AQUÍ: Actualizar 'valor' explícitamente si existe
                        valor: finalDbData.valor !== undefined ? finalDbData.valor : n.data.valor,
                        // Guardamos la data fresca en originalData
        ...
        // 5. Refresco de Grafo (Solo para Pasos)
        // Esto asegura que si cambiaste personaje, recompensa o conexiones, se redibuje.
        if (node.type === 'step' && activeHistoriaId) {
             await loadStoryGraph(activeHistoriaId);
        } else {
             // Feedback visual sencillo para otros nodos
             // Puedes quitar el alert si te resulta molesto
  ...
  const onConnect = useCallback(async (params: Connection) => {
      ...
      if (sourceNode.type === 'step' && targetNode.type === 'step') {
           ...
           if (sData.tipo_paso === 'narrativo') {
               ...
               loadStoryGraph(activeHistoriaId as number);
           }
      }
    ...
    const handleCreateAsset = async (type: string) => { 
        ...
        try {
            ...
            // Ubicación
                ...
                // 1. Pedir Descripción (opcional)
                const desc = prompt("Descripción de la ubicación:", "") || "";

                // 2. Pedir Tipo (con valor por defecto)
            ...
            // Recompensa
                ...
                // 1. Pedir Valor XP (con valor por defecto 10)
                const xpInput = prompt("¿Valor en XP de la recompensa?", "10");
                // Si cancelan o dejan vacío, asumimos 0 o 10, aquí parseamos a entero
                const xpVal = xpInput ? parseInt(xpInput) : 10;

                // 2. Pedir Tipo de Recompensa
                let tipoReward = prompt("Tipo de recompensa (ej: xp, item, moneda, medalla):", "xp");
                if (!tipoReward) tipoReward = 'xp'; // Default
            ...
            // Recurso Multimedia
                ...
                // Segundo prompt para pedir el tipo rápido
                let mediaType = prompt("¿Tipo de recurso? (imagen, video, audio, 3d_model)", "imagen");
                // Validación básica, si lo dejan vacío o cancelan, poner 'imagen'
                if (!mediaType) mediaType = 'imagen'; 
                
                res = await supabase.from('recursomultimedia').insert({ 
                    tipo: mediaType, 
                    archivo: name,
                    metadatos: {} // Iniciamos vacío
                }).select();
            }

            if (res?.data) {
                await loadGlobalAssets(); // Recargar listas
      ...
      <div className="canvas-body">
         ...
         <div className="reactflow-wrapper" ref={reactFlowWrapper}>
            <ReactFlow 
                ...
                onNodeDragStop={onNodeDragStop} // <--- CONECTADO
            ...
            >
                <Background gap={16} color="#444" style={{backgroundColor:'#1a1a1a'}} />
